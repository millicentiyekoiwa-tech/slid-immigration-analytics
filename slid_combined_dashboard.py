...
def ins(t): st.markdown(f'<div class="ins">💡 {t}</div>',
                         unsafe_allow_html=True)
def wrn(t): st.markdown(f'<div class="wrn">⚠ {t}</div>',
                         unsafe_allow_html=True)
def fnd(t): st.markdown(f'<div class="fnd">📌 {t}</div>',
                         unsafe_allow_html=True)
...
df             = load_rp()
pp, office     = load_pp()
lr_,sc_,lc_,lp_,meta_,ols_p,ok = load_models()
...
if page == "📊 Overview":
    ...
    with c1:
        ...
        ins("Passport volumes are 5–10× higher than permit applications.")
    ...
    with c2:
        ...
        ins("Residency permit revenue shows stronger month-on-month growth through June 2026.")
    ...
    with c4:
        ...
        wrn("Three of five delivery metrics off-target. Online adoption and decentralisation are the most urgent gaps.")
...
# ════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        fig = go.Figure()
        for col,clr in [("Ord",P["steel"]),("Svc",P["teal"]),
                        ("Dip",P["gold"])]:
            names = {"Ord":"Ordinary","Svc":"Service","Dip":"Diplomatic"}
            fig.add_trace(go.Scatter(
                x=fpp2["MF"],y=fpp2[col],name=names[col],
                mode="lines+markers",
        ...
        st.plotly_chart(fig, use_container_width=True)
        wrn("Service passport production collapsed 96% between March (108) and May (7). Requires investigation with SLID.")

    with c2:
        sec("Passport Type — Volume & Revenue Share")
        td = pd.DataFrame({
            "Type":["Ordinary","Service","Diplomatic"],
            "Vol" :[fpp2["Ord"].sum(),fpp2["Svc"].sum(),fpp2["Dip"].sum()],
            "Rev" :[fpp2["Ord"].sum()*100,fpp2["Svc"].sum()*100,0]
        })
    ...
    with c3:
        ...
        st.plotly_chart(fig3, use_container_width=True)
        ins(f"Highest gain: July. Steepest decline: May. No consistent trend — demand is highly variable.")
    ...
    with cm1:
        # Use scatter_geo - no token needed
        fig5 = px.scatter_geo(
    ...
    with cm2:
        ...
        fig6 = go.Figure(go.Bar(
            ...
            orientation="h",
            text=office.sort_values("Total")["Pct"]\
        ...
        wrn("Freetown handles 98.18% of all production — a critical single point of failure.")
        fnd("Online channel: only 130 passports (0.33%) — negligible digital adoption.")
...
# ════════════════════════════════════════════════════════════
    ...
    with k2:
        ...
        kpi("Delivery Rate",f"{dr3:.1f}%","g" if dr3>70 else "r")
    ...
    with c1:
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(size=8,color=P["gold"])),secondary_y=True)
        L(fig,380,True)
        ...
        fig.update_yaxes(title_text="Revenue ($)",tickformat="$,.0f",
                         ...
                         tickfont=dict(color=P["sub"]))
        st.plotly_chart(fig, use_container_width=True)
        ins("June 2026 — peak month for both volume (2,634) and revenue ($1.49M).")
    ...
    with c4:
        sec("Delivery Rate & Avg Fee by Category")
        fig4 = make_subplots(2,1,
            subplot_titles=["Delivery Rate (%)","Avg Fee ($)"],
            vertical_spacing=0.16)
        ...
        for a in fig4.layout.annotations:
            a.font.color = P["sub"]
        st.plotly_chart(fig4, use_container_width=True)
        ins("Category B generates highest revenue at $709 avg fee. Category D lowest at $213.")
    ...
    with c5:
        ...
        st.plotly_chart(fig5, use_container_width=True)
        ins("Monday is busiest. Tuesday anomalously low — worth investigating with SLID.")

    with c6:
        sec("Applicant Segments — K-Means Clusters")
        cl = pd.DataFrame({
        ...
        for i,row in cl.iterrows():
            fig6.add_trace(go.Scatter(
                ...
                hovertemplate=(
                    ...
                    f"Delivery: {row['DR']}%<extra></extra>")))
        L(fig6,320,show_legend=False)
        fig6.update_xaxes(title_text="Average Fee ($)")
        fig6.update_yaxes(title_text="Delivery Rate (%)")
        st.plotly_chart(fig6, use_container_width=True)
        wrn("Cluster 2 — 1,649 applicants paid $744 avg in full but received NO permit. "
...
# ════════════════════════════════════════════════════════════
    ...
    with pc1:
        lam = st.number_input("Daily applications (λ)",1,400,51,1,key="lam")
    with pc2:
        s_in = st.number_input("Officers (s)",1,20,3,1,key="sin")
    with pc3:
        mu_in = st.number_input("Apps/officer/day (μ)",1,50,5,1,key="muin")
    ...
    if m_["stable"]:
        ...
        st.markdown(f"""<div class="pr">
          ...
          Need <b>{extra} more officer(s)</b></p>
        </div>""", unsafe_allow_html=True)

    st.markdown("")
    c1,c2 = st.columns(2)
    with c1:
        sec("AS-IS vs TO-BE Processing Time")
        demands = {"Jan\n(Low)":11.62,"Feb":37.96,"Mar":37.93,
                   "Apr":55.43,"May":68.26,"Jun\n(Peak)":87.80,
                   "Jul":52.87,"Aug":42.67}
        scenarios = {
            "AS-IS (s=3,μ=5)"        :(3,5,  P["red"]),
            "Min Viable (s=6,μ=15)"  :(6,15, P["gold"]),
            "Recommended (s=8,μ=12)" :(8,12, P["steel"]),
            "Optimal (s=10,μ=10)"    :(10,10,P["teal"]),
        }
        fig = go.Figure()
        ...
        L(fig,420,True)
        fig.update_yaxes(title_text="Total Days (W)",range=[0,6])
        fig.update_xaxes(title_text="Month")
        st.plotly_chart(fig, use_container_width=True)
        ins("Recommended (s=8, μ=12) stays stable and within the 1-day target across all months including June peak.")

    with c2:
        sec(f"Sensitivity — Processing Time (days) | λ={lam}/day")
        sr=list(range(3,14))
        mr=[5,8,10,12,15,20]
        hd=[]
        for sc in sr:
            ...
            for mv in mr:
                r=mms(lam,mv,sc)
                row[f"μ={mv}"]=round(r["W"],3) if r["stable"] else None
            hd.append(row)
    ...
    for lb,(sc,mv,_) in scenarios.items():
        ...
        pm=mms(87.80,mv,sc)
        rows.append({
            "Scenario":lb,"Servers":sc,"μ":mv,
            "Avg W":f"{am['W']:.4f}d" if am["stable"] else "UNSTABLE",
            "Peak W":f"{pm['W']:.4f}d" if pm["stable"] else "UNSTABLE",
            "✓":("✅" if am["stable"] and pm["stable"] else "❌")
        })
    st.dataframe(pd.DataFrame(rows),
                 use_container_width=True,hide_index=True)
    fnd("Digitising identity verification (NCRA API) is the single highest-impact intervention — it raises μ without adding headcount.")
...
# ════════════════════════════════════════════════════════════
    ...
    with c1:
        ...
        fut=pd.DataFrame({"Month":["2026-09","2026-10",
                                   "2026-11","2026-12"],
                          "MI":[9,10,11,12]})
        ...
        fig.add_trace(go.Scatter(
            x=fut["Month"],y=[851150]*4,name="Conservative",
            mode="lines",
        ...
        fig.update_xaxes(title_text="Month")
        st.plotly_chart(fig, use_container_width=True)
        st.caption(f"R²={ols.rsquared:.3f} | Trend: ${ols.params['MI']:,.0f}/month | n=7")

    with c2:
        sec("Passport — Scenario Projection (Aug–Dec 2026)")
        ...
        fig2.add_trace(go.Scatter(
            ...
            fill="toself",fillcolor="rgba(42,157,143,0.07)",
            line=dict(color="rgba(0,0,0,0)"),name="Range"))
        L(fig2,400,True)
        fig2.update_yaxes(tickformat="$,.0f",title_text="Revenue ($)")
        fig2.update_xaxes(title_text="Month")
        st.plotly_chart(fig2, use_container_width=True)
        st.caption("Scenario-based — no OLS. No consistent linear trend in passport production.")

    st.markdown("")
    sec("2026 Combined Revenue — Three Scenarios")
    pa=pp["Rev"].sum(); ra=df["Amount ($)"].sum()
    scens={"Conservative":(pp["Rev"].min()*5,9649100),
           "Midpoint"    :(pp["Rev"].mean()*5,11789332),
           "Optimistic"  :(pp["Rev"].max()*5,13929564)}
    fig3=go.Figure()
    for nm,col in [("Passport Actual",P["steel"]),
    ...
    rows2=[]
    for sc,(pp_,rp_) in scens.items():
        rows2.append({"Scenario":sc,
            "Passport Full Year":f"${pa+pp_:,.0f}",
            "Permit Full Year":f"${rp_:,.0f}",
            "Combined":f"${pa+pp_+rp_:,.0f}"})
    st.dataframe(pd.DataFrame(rows2),
                 use_container_width=True,hide_index=True)
    st.caption("Passport: $100/passport (Ordinary + Service). Diplomatic exempt. Source: SLID official fee schedule.")
...
# ════════════════════════════════════════════════════════════
    ...
    st.markdown(
        ...
        f"<b style='color:{P['text']};'>(AUC=0.867, Accuracy=82.7%)</b> "
        "predicts whether a residency permit application will be delivered "
        "based on characteristics at the point of submission.</p>",
        unsafe_allow_html=True)

    if not ok:
        st.error("Model files not found. Ensure all .joblib files are in the repo.")
        st.stop()
    ...
    with pc1:
        ...
        mon_in =st.selectbox("Month",list(range(1,13)),
        ...
        exp_in =st.slider("Permit Duration (days)",30,400,364,1,key="pexp")
        card_in=st.radio("Card Number Assigned?",
            ["Yes","No"],horizontal=True,key="pcard")
        go_btn=st.button("🔮 Predict Outcome",use_container_width=True)
    ...
    with pc2:
        ...
        if go_btn:
            try:
                ...
                if pd_>=0.75:
                    css,ic,vd,ac=("pg","🟢",
                        "LOW RISK — Likely Delivered",
                        "Process normally.")
                elif pd_>=0.50:
                    css,ic,vd,ac=("py","🟡",
                        "MEDIUM RISK — Monitor Closely",
                        "Flag for follow-up. Check documents.")
                else:
                    css,ic,vd,ac=("pr","🔴",
                        "HIGH RISK — Likely to Stall",
                        "Escalate immediately. Priority review.")

                st.markdown(f"""<div class="{css}">
                  <h2>{ic} {vd}</h2>
                  <h3>Delivery Probability: {pd_*100:.1f}%</h3>
                  <p><b>Action:</b> {ac}</p>
                </div>""", unsafe_allow_html=True)
                ...
                fg=go.Figure(go.Indicator(
                    ...
                    gauge={
                        ...
                        "steps":[
                            {"range":[0,50],"color":"rgba(230,57,70,0.12)"},
                            {"range":[50,75],"color":"rgba(233,196,106,0.12)"},
                            {"range":[75,100],"color":"rgba(42,157,143,0.12)"},
                        ],
                        "threshold":{"line":{"color":P["sub"],"width":2},
                ...
                sec("Feature Influence")
                cdf=pd.DataFrame({
                    "Feature":["Amount","Month","Day","Expiry",
                               "Card","Category","Process"],
                    "Coef":lr_.coef_[0]
                }).sort_values("Coef")
                fc=go.Figure(go.Bar(
            ...
            st.info("Fill in the details and click **Predict Outcome**.")
            sec("Model Performance")
            st.dataframe(pd.DataFrame({
                "Metric":["Accuracy","AUC","Precision (Del.)",
                          "Recall (Del.)","Recall (Not Del.)",
                          "Training Size"],
                "Value":["82.7%","0.867","85%","93%","51%","8,894"]
            }),use_container_width=True,hide_index=True)
            st.markdown("")
            sec("K-Means Cluster Profiles")
            st.dataframe(pd.DataFrame({
                "Cluster":["Cluster 0","Cluster 1",
                           "Cluster 2","Cluster 3"],
                "Size":[2934,6104,1649,373],
                "Avg Fee ($)":[225,709,744,0],
                "Delivery":["67%","100%","0%","47%"],
                "Revenue at Risk":["—","—","$1,226,800","—"],
                "Description":[
                    "Low-fee, Category E — NGOs, Diplomatic",
                    "High-fee, Category B — Fully processed. Core segment.",
                    "High-fee — Paid in full, NO permit issued. $1.23M backlog (Jun–Jul 2026).",
                    "Zero-fee — Diplomatic exemptions (~167-day permits).",
                ]
            }),use_container_width=True,hide_index=True)
            wrn("Cluster 2 is the most critical finding: 1,649 applicants "
                "paid an average of $744 but received no permit. "
                "This is not an analytical error — it is a $1.23M "
                "operational backlog that requires urgent management attention.")
